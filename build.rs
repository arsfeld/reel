...
fn main() {
    ...
    for file in &blueprint_files {
        ...
        println!("cargo:warning=Compiling {} to {:?}", input_path, output_path);
    ...
    std::fs::copy("src/ui/style.css", ui_dir.join("style.css"))
...
<gresources>
  <gresource prefix="/com/github/arsfeld/Reel">
    ...
    std::fs::write(&gresource_path, gresource_content)
        .expect("Failed to write gresource file");
