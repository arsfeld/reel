...
use crate::backends::{BackendManager, traits::{BackendInfo, BackendType}};
...
impl PreferencesWindow {
    ...
    fn create_backend_row(&self, backend_id: &str, info: &BackendInfo) -> adw::ActionRow {
        ...
        let status_icon = gtk4::Image::from_icon_name(
            if info.is_local {
        ...
        status_icon.set_tooltip_text(Some(
            if info.is_local {
                "Local connection"
            } else if info.is_relay {
                "Relay connection"
    ...
    fn confirm_remove_backend(&self, backend_id: &str) {
        let dialog = adw::MessageDialog::builder()
            ...
            .body(&format!("Are you sure you want to remove this backend?\n\nThis will remove all cached data for this backend."))
            .modal(true)
            .transient_for(self)
            .build();
