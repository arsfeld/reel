use gtk4::{self, prelude::*, glib, subclass::prelude::*};
...
impl MovieDetailsPage {
    ...
    async fn display_movie_info(&self, movie: &Movie) {
        ...
        if duration_hours > 0 {
            imp.duration_label.set_text(&format!("{}h {}m", duration_hours, remaining_mins));
        } else {
            imp.duration_label.set_text(&format!("{} min", duration_mins));
        }
    ...
    fn display_stream_info(&self, stream_info: &StreamInfo) {
        ...
        imp.bitrate_label.set_text(&format!("{:.1} Mbps", bitrate_mbps));
        ...
        if stream_info.direct_play {
            imp.container_label.set_text(&format!("{} (Direct Play)", stream_info.container));
        } else {
            imp.container_label.set_text(&format!("{} (Transcode)", stream_info.container));
        }
    ...
    fn update_watched_button(&self, movie: &Movie) {
        ...
        if movie.watched {
            imp.watched_icon.set_icon_name(Some("checkbox-checked-symbolic"));
    ...
    fn on_play_clicked(&self) {
        if let Some(movie) = self.imp().current_movie.borrow().as_ref() {
            if let Some(callback) = self.imp().on_play_clicked.borrow().as_ref() {
                callback(movie);
            }
        }
    }
            ...
            glib::spawn_future_local(async move {
                if let Some(state) = state {
                    ...
                    if let Some((_, backend)) = backend_manager.get_active_backend() {
                        ...
                        match result {
                            Ok(_) => {
                                info!("Successfully updated watch status for movie: {}", movie.title);
                                
                                // Update the movie's watched status
                                if let Some(page) = page_weak.upgrade() {
                                    let mut updated_movie = movie.clone();
                                    updated_movie.watched = !movie.watched;
                                    page.imp().current_movie.replace(Some(updated_movie.clone()));
