...
impl ShowDetailsPage {
    ...
    async fn display_show_info(&self, show: &Show) {
        ...
        // Set seasons count
        imp.seasons_label.set_text(&format!("{} seasons", show.seasons.len()));
                ...
                if let Some((_, backend)) = backend_manager.get_active_backend() {
                    match backend.get_episodes(&show.id, season_number).await {
                        Ok(episodes) => {
                            ...
                            imp.episodes_count_label.set_text(&format!("{} episodes", episodes.len()));
    ...
    fn update_watched_button(&self) {
        ...
        imp.watched_icon.set_icon_name(Some("object-select-symbolic"));
                        ...
                        if let Some((_, backend)) = backend_manager.get_active_backend() {
                            ...
                            match backend.get_episodes(&show_id, season).await {
                                Ok(episodes) => {
                                    ...
                                    for episode in episodes {
                                        if episode.view_count == 0 {
                                            if let Err(e) = backend.mark_watched(&episode.id).await {
                                                error!("Failed to mark episode {} as watched: {}", episode.id, e);
                                            }
                                        }
                                    }
    ...
    where
        F: Fn(&Episode) + 'static,
    {
        self.imp().on_episode_selected.replace(Some(Box::new(callback)));
    }
