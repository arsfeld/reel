...
impl FilterManager {
    ...
    fn filter_by_watch_status(&self, items: Vec<MediaItem>) -> Vec<MediaItem> {
        ...
        match watch_status {
            ...
            WatchStatus::Watched => items.into_iter().filter(|item| item.is_watched()).collect(),
            WatchStatus::Unwatched => items
                .into_iter()
                .filter(|item| !item.is_watched())
                .collect(),
            WatchStatus::InProgress => items
                .into_iter()
            ...
            self.filter_set.get_filter(&FilterType::Year)
        {
            items
                ...
                .filter(|item| {
                    ...
                    if let Some(y) = year {
                        let above_min = min_year.is_none_or(|min| y >= min);
                        let below_max = max_year.is_none_or(|max| y <= max);
                        above_min && below_max
                    } else {
                        false
                    }
            ...
            self.filter_set.get_filter(&FilterType::Rating)
        {
            items
                ...
                .filter(|item| {
                    ...
                    rating.is_some_and(|r| r >= *min_rating)
                })
                .collect()
        } else {
            items
    ...
    pub fn set_sort_order(&mut self, order: SortOrder) {
        self.filter_set
            .set_filter(FilterType::SortOrder, FilterCriterion::SortOrder(order));
    }
}
