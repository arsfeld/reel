...
mod tests {
    ...
    fn init_db_is_idempotent() {
        ...
        init_db(&mut conn).unwrap();
        init_db(&mut conn).unwrap();
        // baseline + downloads, recorded once each despite the double init.
        assert_eq!(row_count(&mut conn, "__diesel_schema_migrations"), 2);
    }
    ...
    fn foreign_schema_is_rebuilt_as_reel_ng() {
        ...
        // Migrations recorded (baseline + downloads); foreign data did not
        // carry over.
        assert_eq!(row_count(&mut conn, "__diesel_schema_migrations"), 2);
    }
